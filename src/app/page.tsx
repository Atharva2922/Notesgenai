...
export default function Home() {
  ...
  // CRITICAL: Early auth check - runs FIRST to prevent content flash
  useEffect(() => {
    if (typeof window === 'undefined') return;
    ...
    try {
      ...
      if (!stored) {
        // No profile found - redirect to login immediately
        router.push('/login');
        return;
      }
...
// If no user profile after auth check, show nothing (redirect is in progress)
if (!userProfile) {
  return null;
}
